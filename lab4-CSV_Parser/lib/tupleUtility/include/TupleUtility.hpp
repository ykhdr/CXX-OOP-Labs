...
Type readValue(const std::string &row)
{
    ...
    if (!(iss >> value))
    {
        throw std::invalid_argument("bad reading type at " + row + " row, ");
    }
...
void fillTuple(std::tuple<Types...> &tp, const std::vector<std::string> &rowCells, size_t_<N>)
{
    try
    {
        std::get<N - 1>(tp) = readValue<std::tuple_element<N - 1, std::tuple<Types...>>::type>(rowCells[N - 1]);
    }
...
void fillTuple(std::tuple<Types...> &tp, const std::vector<std::string> &rowCells, size_t_<1>)
{
    try
    {
        std::get<0>(tp) = readValue<std::tuple_element<0, std::tuple<Types...>>::type>(rowCells[0]);
    }
    catch (const std::exception &ex)
    {
        std::string msg = ex.what();
